...
fn main() {
    ...
    // if protos/* changes in any way, let Cargo know!
    ...
    println!("cargo:rustc-env=PROVISIONERD_RUSTC_VERSION={rust_version}");
    ...
    println!("cargo:rustc-env=PROVISIONERD_BUILD_DATE={build_date}");
    ...
    if let Ok(true) = headfile.try_exists() {
        ...
        println!("cargo:rustc-env=PROVISIONERD_COMMIT_HASH={}", &contents.trim()[0..8]);
    } else {
        println!("cargo:rustc-env=PROVISIONERD_COMMIT_HASH=d1cebae");
    }
    ...
    tonic_build::configure()
        .compile(&["./protos/noelware/provisionerd/v1/provisionerd.proto"], &["./protos"])
        .expect("protobufs to be generated");
}
